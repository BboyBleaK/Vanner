...
import com.example.vanner.HomeActivity;
...
public class Perfil_Usuario extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnRegresar.setOnClickListener(v -> {
            Intent volverIntent = new Intent(Perfil_Usuario.this, HomeActivity.class);
                    ...
                    mDatabase.child("usuarios").child(userId).removeValue()
                            .addOnCompleteListener(task1 -> {
                                if (task1.isSuccessful()) {
                                    Toast.makeText(Perfil_Usuario.this, "Usuario eliminado correctamente", Toast.LENGTH_SHORT).show();
                                    Intent intent = new Intent(Perfil_Usuario.this, MainActivity.class);
                                    startActivity(intent);
                                    finish();
                                } else {
                                    Log.e("VistaPrueba", "Error al eliminar de la base de datos", task1.getException());
                                    Toast.makeText(Perfil_Usuario.this, "Error al eliminar el usuario de la base de datos", Toast.LENGTH_SHORT).show();
                                }
                            });
                } else {
                    Log.e("VistaPrueba", "Error al eliminar de Auth", task.getException());
                    Toast.makeText(Perfil_Usuario.this, "Error al eliminar el usuario de autenticación", Toast.LENGTH_SHORT).show();
                }
            });
        } else {
